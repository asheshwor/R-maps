...
source("C:/Users//Documents/R_source/fort.R") #for solving fortify error
...
#* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
# The arrivals data have been made from 
# a table in Nepal Tourism Statistics 2012, pdf available at
# http://www.tourism.gov.np/uploaded/TourrismStat2012.pdf
# A few edits have been done on the names of the countries to match
tournp <- "tourismNepal.xlsx"
np.data <- read.xlsx(tournp, 1) #read excel sheet 1
np.data <- np.data[!(np.data$Location == "NA"),]
np.data <- np.data[with(np.data, order(-Total, Location)),] #remove 4 debug
#Since nepal has open border with India, I want to map the countries 
#excluding India
np.data <- np.data[!(np.data$Location == "India"),]
#the origin point of the lines
p1 <- c(84.12, 28.39) #center of Nepal, sort of
#countries with lat long from:
...
np.merged <- np.merged[with(np.merged, order(-Total, Country)),]
...
loc.geo.df$location <- np.merged$Location
...
#* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
mapLabel <- "Arrival figures from http://www.tourism.gov.np/uploaded/TourrismStat2012.pdf \n Country boundaries from NaturalEarthData.com \n Location of cities from NaturalEarthData.com & geonames.org"
mapTitle <- "2012 tourist arrivals in Nepal by top source countries excluding India"
noteText <- "[ Code at https://github.com/asheshwor/R-maps/blob/master/02_great-circle-map.R ]"
...
ggplot() +
  ...
  geom_polygon(aes(long.recenter,lat,group=group.regroup), 
               size = 0.1, fill=NA, colour = "skyblue3",
  ...
  guides(alpha = "none") +
  scale_colour_gradient(high="dodgerblue3", low="red1", trans = "log", #log scale
                        name="Arrivals", labels = comma,
